...
class WeatherDataApi3MakeError
{
    public static function call(?WeatherDataApi3Context $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new WeatherDataApi3Context([], null);
        }
        $op = $ctx->op ?? new WeatherDataApi3Operation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new WeatherDataApi3Result([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof WeatherDataApi3Error) ? $err->msg : (string)$err;
        ...
        $sdk_err = new WeatherDataApi3Error('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof WeatherDataApi3Error) {
