...
//    only - a typed WeatherDataApi3Context there would make every ctx
...
final class WeatherDataApi3Omni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new WeatherDataApi3Context($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new WeatherDataApi3Spec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new WeatherDataApi3Result($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new WeatherDataApi3Error('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new WeatherDataApi3Response($clean['response']);
    ...
    public static function err_from_map($m): ?WeatherDataApi3Error
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new WeatherDataApi3Error(is_string($code) ? $code : '', $msg);
    }
