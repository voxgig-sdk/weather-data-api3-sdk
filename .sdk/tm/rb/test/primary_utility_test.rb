...
class PrimaryUtilityTest < Minitest::Test
  ...
  def test_feature_add_basic
    ...
    feature = WeatherDataApi3BaseFeature.new
  ...
  def test_feature_hook_basic
    hook_client = WeatherDataApi3SDK.test(nil, nil)
  ...
  def test_feature_init_basic
    init_client = WeatherDataApi3SDK.test(nil, nil)
  ...
  def test_fetcher_live
    calls = []
    live_client = WeatherDataApi3SDK.new({
  ...
  def test_fetcher_blocked_test_mode
    blocked_client = WeatherDataApi3SDK.new({
      "base" => "http://localhost:8080",
      "system" => {
        "fetch" => ->(url, fetchdef) {
          return {}, nil
  ...
  def test_make_fetch_def_basic
    ctx = make_test_full_ctx(@client, @utility)
    ctx.spec = WeatherDataApi3Spec.new({
      "base" => "http://localhost:8080",
      "prefix" => "/api",
      "path" => "items/{id}",
      "suffix" => "",
  ...
  def test_make_result_no_result
    ctx = make_test_full_ctx(@client, @utility)
    ctx.spec = WeatherDataApi3Spec.new({ "step" => "start" })
    ctx.result = nil
  ...
  def test_make_spec_basic
    ...
    spec_client = WeatherDataApi3SDK.test(nil, setup_opts)
  ...
  def test_make_url_basic
    runset(get_spec(@primary, "makeUrl", "basic")) do |entry|
      ctxmap = entry["ctx"] || {}
      ctx = make_ctx_from_map(ctxmap, @client, @utility)
      ctx.result = WeatherDataApi3Result.new({}) unless ctx.result
  ...
  def test_operator_basic
    runset(get_spec(@primary, "operator", "basic")) do |entry|
      in_val = entry["in"] || {}
      op = WeatherDataApi3Operation.new(in_val)
      {
        "entity" => op.entity,
        "name" => op.name,
        "input" => op.input,
  ...
  def test_prepare_auth_basic
    ...
    auth_client = WeatherDataApi3SDK.test(nil, setup_opts)
  ...
  def make_ctx_from_map(ctxmap, client, utility)
    ...
    ctx = WeatherDataApi3Context.new(ctxmap, nil)
    ...
    if ctxmap["spec"].is_a?(Hash)
      ctx.spec = WeatherDataApi3Spec.new(ctxmap["spec"])
    end

    # Handle result from JSON map
    if ctxmap["result"].is_a?(Hash)
      res_map = ctxmap["result"]
      ctx.result = WeatherDataApi3Result.new(res_map)
      if res_map["err"].is_a?(Hash) && res_map["err"]["message"].is_a?(String)
        ctx.result.err = WeatherDataApi3Error.new("", res_map["err"]["message"])
      end
    end
    ...
    if ctxmap["response"].is_a?(Hash)
      ...
      ctx.response = WeatherDataApi3Response.new(resp_map)
      if resp_map["body"]
  ...
  def err_from_map(m)
    ...
    code = m["code"] || ""
    WeatherDataApi3Error.new(code, msg)
  end
end
