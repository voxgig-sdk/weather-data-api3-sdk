...
class PrimaryUtilityTest < Minitest::Test
  ...
  def setup
    @runner = WeatherDataApi3Omni.make_runner(TEST_JSON_FILE, WeatherDataApi3SDK.test(nil, nil))
  ...
  def err_from_map(m)
    ...
    WeatherDataApi3Error.new(m["code"] || "", msg)
  end
  ...
  def test_make_point_basic
    ...
    subject = lambda do |ctx|
      ...
      err.nil? ? point : WeatherDataApi3Omni.errify(err)
    end
  ...
  def test_operator_basic
    subject = lambda do |vin|
      vin = {} unless vin.is_a?(Hash)
      op = WeatherDataApi3Operation.new(vin)
      {
        "entity" => op.entity,
        "name" => op.name,
        "input" => op.input,
