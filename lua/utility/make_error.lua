...
local function make_error_util(ctx, err)
  ...
  local msg = "WeatherDataApi3SDK: " .. opname .. ": " .. errmsg
  ...
  local sdk_err = WeatherDataApi3Error.new("", msg, ctx)
  sdk_err.result = ctx.utility.clean(ctx, result)
  sdk_err.spec = ctx.utility.clean(ctx, spec)

  if type(err) == "table" and getmetatable(err) == WeatherDataApi3Error then
