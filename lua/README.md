# WeatherDataApi3 Lua SDK



The Lua SDK for the WeatherDataApi3 API — an entity-oriented client using Lua conventions.

It exposes the API as capitalised, semantic **Entities** — e.g. `client:Forecast()` — each with the same small set of operations (`load`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
GitHub release tag (`lua/vX.Y.Z`, see [Releases](https://github.com/voxgig-sdk/weather-data-api3-sdk/releases)),
...
```
lua/
├── weather-data-api3_sdk.lua    -- Main SDK module
...
The main module (`weather-data-api3_sdk`) exports the SDK constructor
...
forecast:load({ latitude = 1, longitude = 1 })
