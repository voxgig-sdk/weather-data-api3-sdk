...
//     (\`new WeatherDataApi3SDK(...)\` / \`WeatherDataApi3SDK.test(...)\` become
//     \`WeatherDataApi3SDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'WeatherDataApi3SDK'
...
// rewrite real client construction (\`new WeatherDataApi3SDK(...)\` and any
// \`WeatherDataApi3SDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+WeatherDataApi3SDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/WeatherDataApi3SDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
