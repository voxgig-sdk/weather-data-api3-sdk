...
describe('ForecastDirect', async () => {
  ...
  test('direct-exists', async () => {
    const sdk = new WeatherDataApi3SDK({
  ...
  test('direct-load-forecast', async (t: any) => {
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-forecast', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['WEATHER_DATA_API3_TEST_FORECAST_ENTID']
