# WeatherDataApi3 TypeScript SDK Reference

Complete API reference for the WeatherDataApi3 TypeScript SDK.
...
```ts
new WeatherDataApi3SDK(options?: object)
```
...
#### `WeatherDataApi3SDK.test(testopts?, sdkopts?)`
...
**Returns:** `WeatherDataApi3SDK` instance in test mode.
...
Alias for `WeatherDataApi3SDK.test()`.
...
```ts
const result = await client.Forecast().load({ latitude: 1, longitude: 1 })
```
...
Return the parent `WeatherDataApi3SDK` instance.
