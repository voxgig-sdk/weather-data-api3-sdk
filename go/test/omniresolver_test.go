...
import (
	...
	omni "github.com/voxgig-sdk/weather-data-api3-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.WeatherDataApi3Error); is {
				out := map[string]any{
					"name":    "WeatherDataApi3Error",
...
func omniCtx(arg any, client *sdk.WeatherDataApi3SDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.WeatherDataApi3SDK); is {
			client = live
