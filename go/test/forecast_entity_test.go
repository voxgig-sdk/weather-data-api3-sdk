...
func TestForecastEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		forecastRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.forecast")))
...
func forecastBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("WEATHER_DATA_API3_TEST_FORECAST_ENTID")
	...
	idmapResolved := core.ToMapAny(env["WEATHER_DATA_API3_TEST_FORECAST_ENTID"])
