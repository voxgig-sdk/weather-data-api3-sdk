...
//
// NewWeatherDataApi3SDK, not TestSDK: the `test` feature is transport: 'base' and
// REPLACES the transport, so a client in test mode would shadow the script.
func fcClient(kase map[string]any) *sdk.WeatherDataApi3SDK {
	res, _ := kase["res"].([]any)
	opts := map[string]any{
	...
	return sdk.NewWeatherDataApi3SDK(opts)
}
...
func fcCandidates(client *sdk.WeatherDataApi3SDK) []fcOp {
	out := []fcOp{}
...
func fcInvoke(client *sdk.WeatherDataApi3SDK, op fcOp, ctrl map[string]any) error {
	cv := reflect.ValueOf(client)
	ct := cv.Type()
	mapType := reflect.TypeOf(map[string]any{})
...
// new go here.
func fcRecord(client *sdk.WeatherDataApi3SDK, name string) any {
