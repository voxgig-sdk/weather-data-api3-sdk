# WeatherDataApi3 SDK test runner
...
class WeatherDataApi3TestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            WeatherDataApi3TestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = WeatherDataApi3TestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = WeatherDataApi3TestRunner.getenv("WEATHER_DATA_API3_TEST_LIVE")
        override = WeatherDataApi3TestRunner.getenv("WEATHER_DATA_API3_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = WeatherDataApi3TestRunner.getenv(key)
        ...
        explain = WeatherDataApi3TestRunner.getenv("WEATHER_DATA_API3_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["WEATHER_DATA_API3_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if WeatherDataApi3TestRunner._test_control is not None:
            return WeatherDataApi3TestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                WeatherDataApi3TestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            WeatherDataApi3TestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = WeatherDataApi3TestRunner.load_test_control()
...
def load_env_local():
    WeatherDataApi3TestRunner.load_env_local()


def env_override(m):
    return WeatherDataApi3TestRunner.env_override(m)


def entity_data(v):
    return WeatherDataApi3TestRunner.entity_data(v)


def entity_list_to_data(lst):
    return WeatherDataApi3TestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return WeatherDataApi3TestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return WeatherDataApi3TestRunner.load_test_control()


def live_client_options():
    return WeatherDataApi3TestRunner.live_client_options()


def live_delay_ms():
    return WeatherDataApi3TestRunner.live_delay_ms()
