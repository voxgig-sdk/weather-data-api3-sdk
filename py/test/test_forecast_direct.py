...
class TestForecastDirect:
    ...
    def test_should_direct_load_forecast(self):
        setup = _forecast_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-forecast", "live" if setup["live"] else "unit")
        if _skip:
