# weather-data-api3-mcp

[MCP](https://modelcontextprotocol.io) server exposing the WeatherDataApi3 SDK as
two agent tools — `weather-data-api3_list` and `weather-data-api3_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/weather-data-api3-mcp)
make build

# 2. Provide credentials via the environment
export WEATHER_DATA_API3_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user weather-data-api3 \
  -- /absolute/path/to/weather-data-api3-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./weather-data-api3-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// weather-data-api3_list: first page of records
{ "entity": "forecast" }
{ "entity": "forecast", "query": { } }

// weather-data-api3_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/weather-data-api3-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user weather-data-api3 \
     -- "$PWD"/dist/*/weather-data-api3-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `weather-data-api3_list` and `weather-data-api3_load` tools now appear
   in new sessions. Ask the agent to *"list forecast using weather-data-api3"*
   and it calls `weather-data-api3_list` with `{"entity":"forecast"}`.
...
```sh
export WEATHER_DATA_API3_APIKEY=sk_live_xxx            # API key
export WEATHER_DATA_API3_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `weather-data-api3_list` tool
...
### Call the `weather-data-api3_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `weather-data-api3_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `weather-data-api3_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `WEATHER_DATA_API3_APIKEY` | API key sent with every request. |
| `WEATHER_DATA_API3_BASE` | Optional override of the API base URL. |
...
```sh
./weather-data-api3-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"weather-data-api3_load","arguments":{"entity":"forecast","query":{"id":1}}}}'
```
