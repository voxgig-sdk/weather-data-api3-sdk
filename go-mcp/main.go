// Command WeatherDataApi3-mcp is an MCP server exposing the WeatherDataApi3
// SDK as MCP tools.
//
// Two tools are registered:
//
//	WeatherDataApi3_list  — list records of an entity
//	WeatherDataApi3_load  — load a single record of an entity
//
...
func main() {
	...
	// Configure from the environment: WEATHER_DATA_API3_APIKEY carries the API key and
	// WEATHER_DATA_API3_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("WEATHER_DATA_API3_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("WEATHER_DATA_API3_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewWeatherDataApi3SDK(opts)
