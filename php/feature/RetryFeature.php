...
// WeatherDataApi3 SDK retry feature
...
class WeatherDataApi3RetryFeature extends WeatherDataApi3BaseFeature
{
    ...
    public function init(WeatherDataApi3Context $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (WeatherDataApi3Context $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(WeatherDataApi3Context $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(WeatherDataApi3Context $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
