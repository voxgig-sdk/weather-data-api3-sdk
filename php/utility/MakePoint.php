...
// WeatherDataApi3 SDK utility: make_point

require_once __DIR__ . '/../core/Helpers.php';

class WeatherDataApi3MakePoint
{
    public static function call(WeatherDataApi3Context $ctx): array
    {
        if (isset($ctx->out['point'])) {
            ...
            if ($ctx->out['point'] instanceof WeatherDataApi3Error) {
        ...
        if (count($op->points) === 1) {
            ...
            foreach ($op->points as $p) {
                $select_def = WeatherDataApi3Helpers::to_map(\Voxgig\Struct\Struct::getprop($p, 'select'));
                $found = true;
            ...
            if ($reqselector) {
                $req_action = \Voxgig\Struct\Struct::getprop($reqselector, '$action');
                if ($req_action && $point) {
                    $point_select = WeatherDataApi3Helpers::to_map(\Voxgig\Struct\Struct::getprop($point, 'select'));
