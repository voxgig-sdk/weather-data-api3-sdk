...
// WeatherDataApi3 SDK feature test
//
...
class FtCtrl extends WeatherDataApi3Control
{
...
class FtClient
{
    ...
    private WeatherDataApi3Utility $utility;

    public function __construct(WeatherDataApi3Utility $utility, array $options)
    {
    ...
    public function get_utility(): WeatherDataApi3Utility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (WeatherDataApi3Context $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public WeatherDataApi3Context $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (WeatherDataApi3Context $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = WeatherDataApi3Config::make_feature($name);
    ...
    private static function buildUrl(WeatherDataApi3Spec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof WeatherDataApi3Error) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new WeatherDataApi3Response($fetched);
        }
    ...
    private function fail(WeatherDataApi3Context $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (WeatherDataApi3Context $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
