...
class WeatherDataApi3SDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = WeatherDataApi3Helpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = WeatherDataApi3Helpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!WeatherDataApi3Features::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, WeatherDataApi3Features::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return WeatherDataApi3Utility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = WeatherDataApi3Helpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = WeatherDataApi3Helpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = WeatherDataApi3Helpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new WeatherDataApi3Error($op . "_allow",
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = WeatherDataApi3Helpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new WeatherDataApi3Error("graphql_error",
                "WeatherDataApi3SDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public static function test(?array $testopts = null, ?array $sdkopts = null): self
    {
        ...
        $sdk = new WeatherDataApi3SDK($sdkopts);
        $sdk->mode = "test";
        return $sdk;
    }
}
