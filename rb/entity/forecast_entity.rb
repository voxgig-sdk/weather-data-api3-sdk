...
class ForecastEntity
  ...
  def data_set(args)
    if args
      @_data = WeatherDataApi3Helpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = WeatherDataApi3Helpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  #
  # @param reqmatch [ForecastLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Forecast.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Forecast, Hash] the loaded Forecast; raises WeatherDataApi3Error on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = WeatherDataApi3Helpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
