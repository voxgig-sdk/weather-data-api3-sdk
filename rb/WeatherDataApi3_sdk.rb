...
require_relative 'WeatherDataApi3_types'
...
class WeatherDataApi3SDK
  ...
  def initialize(options = {})
    ...
    utility = WeatherDataApi3Utility.new
    @_utility = utility
    ...
    feature_opts = WeatherDataApi3Helpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = WeatherDataApi3Helpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, WeatherDataApi3Features.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    WeatherDataApi3Utility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = WeatherDataApi3Helpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = WeatherDataApi3Helpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = WeatherDataApi3Helpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => WeatherDataApi3Error.new(
        "#{op}_allow",
        "WeatherDataApi3SDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    rescue WeatherDataApi3Error => err
    ...
    if fetched.is_a?(Hash)
      status = WeatherDataApi3Helpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = WeatherDataApi3Error.new(
        "graphql_error", "WeatherDataApi3SDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = WeatherDataApi3SDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
