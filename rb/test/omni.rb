...
module WeatherDataApi3Omni
  ...
  class ObjView < ::Hash
    ...
    def view_wrap(name)
      ...
      return val if WeatherDataApi3Omni.plainval?(val) || seen.key?(val.object_id)
    ...
    def view_attrname(key)
      ...
      snake = WeatherDataApi3Omni.underscore(name)
    ...
    def keys
      out = view_names.map { |n| WeatherDataApi3Omni.camelize(n) }
    ...
    def []=(key, val)
      ...
      t = @view_target
      name = view_attrname(key) || WeatherDataApi3Omni.underscore(key.to_s)
      setter = "#{name}="
  ...
  def enrich(ctxmap, ctx)
    ...
    ctx.spec = WeatherDataApi3Spec.new(spec_map) if spec_map.is_a?(::Hash)
    ...
    if res_map.is_a?(::Hash)
      ...
      if err_map.is_a?(::Hash) && err_map['message'].is_a?(::String) &&
        ...
        ctx.result.err = WeatherDataApi3Error.new('', err_map['message'])
      end
    end
  ...
  def sdkhooks(sdk)
    ...
    errify = ->(err) { WeatherDataApi3Omni.errify(err) }
