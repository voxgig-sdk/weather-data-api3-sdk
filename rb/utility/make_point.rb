...
module WeatherDataApi3Utilities
  MakePoint = ->(ctx) {
    if ctx.out["point"]
      ...
      return nil, preset if preset.is_a?(WeatherDataApi3Error)
      ctx.point = preset
    ...
    if op.points.length == 1
      ...
      matched = false
      op.points.each do |p|
        select_def = WeatherDataApi3Helpers.to_map(VoxgigStruct.getprop(p, "select"))
        found = true
      ...
      if reqselector
        req_action = VoxgigStruct.getprop(reqselector, "$action")
        if req_action && point
          point_select = WeatherDataApi3Helpers.to_map(VoxgigStruct.getprop(point, "select"))
