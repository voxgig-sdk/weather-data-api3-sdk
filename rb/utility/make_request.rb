...
module WeatherDataApi3Utilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = WeatherDataApi3Response.new({})
    ...
    if fetch_err
      ...
      response = WeatherDataApi3Response.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = WeatherDataApi3Response.new(fetched)
    else
