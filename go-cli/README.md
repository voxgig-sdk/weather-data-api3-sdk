# weather-data-api3-cli

boru-driven command-line client **and** interactive REPL for the WeatherDataApi3
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/weather-data-api3-cli)
make build

# 2. See usage (words, entities, env vars)
./weather-data-api3-cli --help

# 3. Provide credentials once, via the environment
export WEATHER_DATA_API3_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./weather-data-api3-cli load 1 forecast            # {id:1} shorthand
./weather-data-api3-cli load '{id:1}' forecast       # explicit match map

# 5. Override the API base URL for a single call
WEATHER_DATA_API3_BASE=https://api.example.com ./weather-data-api3-cli load 1 forecast

# 6. No arguments -> interactive REPL
./weather-data-api3-cli
weather-data-api3> load 1 forecast
weather-data-api3> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/weather-data-api3-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/weather-data-api3-cli load 1 forecast
   ```
...
```sh
./weather-data-api3-cli load 1 forecast          # scalar shorthand for {id:1}
./weather-data-api3-cli load '{id:1}' forecast     # explicit match map
```
...
./weather-data-api3-cli load 1 forecast
```
...
Run with no arguments to open a REPL (prompt `weather-data-api3>`). Each line is
evaluated as its own boru expression:

```text
$ ./weather-data-api3-cli
weather-data-api3> load 1 forecast
weather-data-api3> /help
weather-data-api3> /quit
```
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/weather-data-api3-cli`. |
...
(`./weather-data-api3-cli <expr>`) and interactively (the REPL), and expressions compose the
